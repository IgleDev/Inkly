...
export default function CreateModalBlock() {
    ...
    const modalBack = useAppStore(state => state.modalBack);
    const closeModalBack = useAppStore(state => state.closeModalBack);
    const openModalBack = useAppStore(state => state.openModalBack);

    const isOpen = modalBack && !showModal
    ...
    return (
        <div>
            <button onClick={handleOpen} className="text-sm text-gray-500 hover:text-[#C53F56] transition-colors mb-4 p-5"><ArrowLeftIcon /> Volver</button>
